  ...
  <section class="relative py-16 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto text-center">
      <h1 class="text-4xl md:text-6xl font-bold text-white mb-6">
        <span class="bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 bg-clip-text text-transparent">Découvrez</span><br>
        <span class="text-white">nos annonces</span>
      </h1>
      <p class="text-xl text-white/80 mb-8 max-w-2xl mx-auto">
        Trouvez exactement ce que vous cherchez parmi des milliers d'annonces. Achetez, vendez, échangez en toute simplicité.
      </p>
      ...
      <form action="{{ route('home') }}" method="GET"
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div class="md:col-span-2">
            ...
            <input type="text" id="search" name="search" value="{{ request('search') }}" placeholder="Ex: iPhone, Voiture..."
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-6 flex flex-wrap gap-2 text-sm">
        @foreach(['search','category_id','localisation','prix_min','prix_max'] as $key)
          @if(request($key))
            <span class="bg-white/10 backdrop-blur border border-white/20 px-3 py-1 rounded-full text-white flex items-center space-x-2">
        ...
        @if(request()->hasAny(['search','category_id','localisation','prix_min','prix_max']))
          <a href="{{ route('home') }}" class="bg-red-500/20 border border-red-500/30 px-3 py-1 rounded-full text-red-200 hover:bg-red-500/30">Effacer tout</a>
        @endif
      </div>
          ...
          @foreach($annonces as $annonce)
            <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/30">
              <div class="relative h-48">
                ...
                @endif
                <div class="absolute top-3 left-3 bg-white/20 backdrop-blur border border-white/30 px-2 py-1 rounded text-xs text-white font-medium">
              ...
              <div class="p-4">
                ...
                <a href="{{ route('annonces.show', $annonce) }}" class="block w-full bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-center py-2 rounded-lg text-white font-medium transition transform hover:scale-105">
                  Voir les détails
                </a>
              </div>
            </div>
      ...
      @else
        <div class="text-center py-16">
          <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-8 max-w-md mx-auto">
            ...
            @auth
              <a href="{{ route('annonces.create') }}" class="bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 rounded-lg text-white">Publier la première</a>
            @endauth
          </div>
        </div>
      @endif
...
@push('scripts')
  @vite(['resources/js/script_annonces.js'])
@endpush